...
public class PostfixDriver {
	 ...
	 * 
	 * @return Next command. If there are three erroneous attempts, the quit command
	 *         is returned.
	 * @throws IOException If there is an i/o error in reading the next command.
	 */
	static Command getCommand() throws IOException {
		int tries = 3;
		while (tries > 0) {
			System.out.println();
	 ...
	 * 
	 * @throws IOException If there is an i/o error in reading the postfix
	 *                     expression.
	 */
	static void startEval() throws IOException {
	 ...
	 * @throws IOException                If there is any i/o error in the
	 *                                    evaluation process.
	 * @throws IllegalExpressionException If the given expression is not legal
	 *                                    postfix.
	 */
	public static void main(String[] args) throws IOException, IllegalExpressionException {
		...
		while ((cmd = getCommand()) != Command.quit) {
			switch (cmd) {
			case runall:
				float result = myEvaluator.runAll(pw);
				System.out.println(String.format("     Result => %.2f", result));
				break;
			case runsome:
				System.out.print("Evaluate how many steps? => ");
				System.out.flush();
				int howManySteps = Integer.parseInt(stdbr.readLine());
